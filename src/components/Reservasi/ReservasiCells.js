...
export default function ReservasiCells({ columns, users, onUpdate }) {
  ...
  const renderCell = React.useCallback((user, columnKey) => {
    ...
    switch (columnKey) {
      case "name":
        return (
          <User avatarProps={{ radius: "lg", src: user.avatar }} description={user?.pasien?.nama} name={user?.pasien?.nama}>
            {user?.pasien?.nama}
          </User>
        );
      case "role":
  ...
  return (
    <div className="w-full flex flex-col gap-4">
      ...
      <MainModal
        ...
        content={<ReservasiForm id={null} onClose={() => setAddPasien(false)} onSuccess={(data) => onUpdate(data)} />}
        showFooter={false}
      />
      <MainModal
        size="md"
        ...
        content={<ReservasiForm id={selectPasien} onClose={() => setUpdatePasien(false)} onSuccess={(data) => onUpdate(data)} />}
        showFooter={false}
      />
